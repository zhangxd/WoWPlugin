...
- 最后更新：2026-05-01（RouteBar 视觉规则细化，补充胶囊分栏、节点高亮与坐标标记）
...
- 导航信息页主视图不再显示完整步骤列表
- 导航信息页主视图不再重复显示路线摘要
- 主视图只显示导航节点串联，不显示额外说明块
- 节点链采用竖向“串站”表达，类似地铁站站点列表，而不是时间线步骤流
- 节点链只保留地图级节点与显式交通枢纽节点，例如：
  - 主城
  - 港口 / 飞艇塔
  - 传送门房 / 公共传送门
  - 炉石落点
  - 法师传送门落点
- 飞行点详情不进入玩家可见节点链；后续若继续补炉石、传送门、飞艇等模态，这些交通枢纽节点要保留
- 节点链不再在玩家可见文本中插入“前往 / 动作说明”分隔；动作感由连线与节点顺序表达
- 左侧节点标记使用通用类型图标区分 `地图 / 传送门 / 飞艇等交通节点`

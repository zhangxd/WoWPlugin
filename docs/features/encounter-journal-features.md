...
- 想在副本详情页确认当前难度的重置时间，并保留 Blizzard 原始掉落列表浏览体验。
...
- 未开启“定位图标常驻显示”时，图钉只在鼠标悬停到可导航列表条目时显示；单击列表条目仍沿用 Blizzard 默认进入详情页行为。

### 3.2 副本详情页增强

- 在详情页标题区域显示当前所选难度的重置时间；没有锁定时显示“重置：无”。
- 详情页不再提供“仅坐骑”按钮或掉落过滤，掉落列表保持 Blizzard 原始内容。
...
- `mountFilterEnabled`
  记忆副本列表“仅坐骑”按钮的上次开关状态。
- `listPinAlwaysVisible`
  控制副本列表中的入口图钉是否常驻显示。

说明：

- 副本列表 CD 叠加与 tooltip 锁定详情固定跟随 `encounter_journal` 模块总开关，不再提供独立设置项。
- 详情页“仅坐骑”设置与功能已移除。
...
| 2026-04-29 | 收口设置与详情页行为：设置页删除 3 个旧选项；列表“仅坐骑”仅保留按钮记忆；详情页“仅坐骑”功能移除 |

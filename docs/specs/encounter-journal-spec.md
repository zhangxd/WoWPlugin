...
- 副本 / 地下城列表条目“进入详情页”的单击 / 双击模式切换，以及图钉焦点 / 悬停显示与常驻显示设置。
...
- 副本列表进入详情页规则固定为：
  - 单击列表行仅更新当前焦点行与图钉显示；
  - 双击同一列表行时沿用 Blizzard 当前默认“进入详情页”行为；
...
8. 未开启“定位图标常驻显示”时，单击某个副本列表条目后，该条目进入焦点态并显示图钉；非焦点条目默认不显示图钉。
9. 未开启“定位图标常驻显示”时，鼠标悬停任意可导航副本列表条目时，即使该条目不是焦点，也会显示图钉。
10. 双击某个副本列表条目后，会进入该副本详情页，沿用 Blizzard 当前默认进入详情页行为。
...
- 当前状态：已完成
- 下一步：在游戏内补齐手工验证，重点确认列表单击焦点 / 双击进入详情页行为与图钉显示表现。

...
| `Toolbox.Navigation` | `Core/API/Navigation.lua` | 地图导航领域 API：统一消费运行时静态路线边，按当前角色职业、阵营、已学法术、已开航点与炉石绑定点过滤和展开路线图；主求解器按最少路径步数工作，并输出逐段方式与经过地图。当前运行时已支持 `taxi / transport / public_portal / hearthstone / class_teleport / class_portal / walk_local`，并已新增 `navigation_walk_components` 正式出口，优先驱动首批主城 / 传送门房 / 港口与飞艇塔 / 常用交通落点的本地接入与显示代理；`areatrigger` 与全世界 `walk component` 真值仍待后续闭合。规划成功后的聊天框排查输出由 `Modules/Navigation/WorldMap.lua` 在调用侧一次性格式化，不把 `RouteBar` 摘要当作调试日志复用。详见 [designs/navigation-design.md](./designs/navigation-design.md)。 |
...
| 2026-05-02 | `navigation` 首批 walk component 出口落地：新增 `navigation_walk_components` 契约、`NavigationWalkComponents.lua`、源侧 override 与 runtime 优先消费；本地步行接入和显示代理优先读取 formal component，缺失时继续 fallback 到 `WalkClusterNodeID / WalkClusterKey` |

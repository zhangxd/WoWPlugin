...
**原则**：可扩展能力经 `RegisterModule` 进入；持久化落在 `ToolboxDB.modules[moduleId]`；设置页由宿主按叶子页组合内容块并统一承载公共启用/调试/重置区，再由各模块通过统一 helper（toggle / choice / menu / multi-select / action / note / custom block）补自己的专属设置区。
...
| `Toolbox.Navigation` | `Core/API/Navigation.lua` | 地图导航领域 API：统一消费运行时静态路线边，按当前角色职业、阵营、已学法术、已开航点与炉石绑定点过滤和展开路线图；主求解器按最少路径步数工作，并输出逐段方式与经过地图。当前运行时已支持 `taxi / transport / public_portal / hearthstone / class_teleport / class_portal / walk_local`，并已新增 `navigation_walk_components` 正式出口，优先驱动首批主城 / 传送门房 / 港口与飞艇塔 / 常用交通落点的本地接入与显示代理；该出口后续只允许由正式来源表全自动推导，不再依赖人工 override 文件。`areatrigger` 与全世界 `walk component` 真值仍待后续闭合。规划成功后的聊天框排查输出由 `Modules/Navigation/WorldMap.lua` 在调用侧一次性格式化，不把 `RouteBar` 摘要当作调试日志复用。详见 [designs/navigation-design.md](./designs/navigation-design.md)。 |
...
| Tooltip 锚点 | `tooltip_anchor` | `modules.tooltip_anchor`（`enabled`/`debug`/`mode`/`offsetX`/`offsetY`） | 归入“界面”叶子页：启用、3 项菜单式锚点模式（`default / cursor / follow`）；调试与“清理并重建”位于页底低频区；保留旧偏移字段兼容但不在本轮设置页继续展开 |
...
| 加载聊天提示 | `chat_notify` | `modules.chat_notify`（`enabled`/`debug`/`prefixColor`/`contentColor`） | 归入“通用”叶子页：最近聊天复制动作、加载提示颜色菜单与必要说明；调试与“清理并重建”位于页底低频区 |
| 冒险指南增强 | `encounter_journal` | `modules.encounter_journal`（`enabled`/`debug`/`mountFilterEnabled`/`listPinAlwaysVisible`）+ `Toolbox.Data.MountDrops` + `Toolbox.Data.InstanceMapIDs` + `Toolbox.Data.NavigationInstanceEntrances` + `Toolbox.EJ` | 归入“冒险手册”叶子页：覆盖副本列表“仅坐骑”、副本列表图钉导航、列表锁定叠加、悬停锁定详情、详情页重置标签，以及 `EJMicroButton` / 小地图“冒险手册” tooltip 锁定摘要。入口导航不新增额外存档字段，运行时按当前 `journalInstanceID` 直接读取规范化导出入口目标，跟随模块总开关。详见 [designs/encounter-journal-design.md](./designs/encounter-journal-design.md)。 |
| 地图导航 | `navigation` | `modules.navigation`（`enabled`/`debug`/`lastTargetUiMapID`/`lastTargetX`/`lastTargetY`）+ `Toolbox.Navigation` + `Toolbox.Data.NavigationMapNodes` + `Toolbox.Data.NavigationMapAssignments` + `Toolbox.Data.NavigationInstanceEntrances` + `Toolbox.Data.NavigationAbilityTemplates` + `Toolbox.Data.NavigationRouteEdges` + `Toolbox.Data.NavigationWalkComponents` | 归入“地图”叶子页：当前导航基线已收敛为“当前角色配置 + 最少路径步数”的多模态路线图，并已在统一静态边表中接入 `taxi / transport / public_portal / hearthstone / class_teleport / class_portal / walk_local`；`navigation_walk_components` 也已正式导出并优先承接首批主城 / 传送门房 / 港口与飞艇塔 / 常用交通落点的本地接入与显示代理。仍待后续补齐的模态主要是 `areatrigger` 与全世界 `walk component` 真值。地图基础节点由 `navigation_map_nodes` 正式导出，`navigation_map_assignments` 当前仅导出 `UiMap <-> MapID` 关系，副本入口外部目标由 `navigation_instance_entrances` 正式导出，能力模板由 `navigation_ability_templates` 正式导出，运行时静态路线边统一由 `navigation_route_edges` 正式导出，步行组件与代理规则由 `navigation_walk_components` 正式导出。自 2026-04-30 起，运行时节点引用与能力模板目标统一切到不透明数字 `NodeID` / `ToNodeID`，本地步行接入优先读取 `NavigationWalkComponents` 的 `PreferredAnchorNodeID / VisibleName / DisplayProxyNodeID`，缺失时再回退到 `WalkClusterNodeID` 兼容链；测试与运行时均通过 `Source + SourceID + Kind` 解析节点，不再依赖旧字符串节点键。顶部路线 UI 的当前确认形态为：三段式起始/当前/终点胶囊 + 只显示状态与步骤进度的标题 + WoW 可落地约束下的竖向导航节点串站视图 + 独立侧贴最近路线抽屉。规划期诊断输出与顶部路线 UI 分离：前者仅在规划完成时一次性输出摘要，后者只负责实时状态展示，不承担实时日志。`navigation_taxi_edges` 只作为 Taxi 来源侧导出，不再由运行时构图链路直接消费；`Region_*`、轨迹坐标与 SafeLoc 坐标不再进入运行时路线图。所有导航运行时数据必须由 DataContracts 契约导出，`NavigationManualEdges.lua` 不得作为运行时数据源。详见 [specs/navigation-spec.md](./specs/navigation-spec.md)、[designs/navigation-design.md](./designs/navigation-design.md)、[plans/navigation-plan.md](./plans/navigation-plan.md)。 |
...
- `RegisterSettings(box)` 统一通过宿主 helper 追加内容：`AddToggleRow`、`AddChoiceRow`、`AddMenuRow`、`AddMultiSelectRow`、`AddActionRow`、`AddNoteRow`、`AddCustomBlock`。两项单选优先 `AddChoiceRow`，三项及以上单选或下拉优先 `AddMenuRow`。
- 模块**不得自行管理纵向游标或手工回写 `box.realHeight`**；宿主 helper 负责累计高度。只有复杂自定义块才通过 `AddCustomBlock` 返回实际高度。
- 行级禁用态、局部刷新与重建统一经宿主处理：仅影响勾选态 / 选中态 / 文案时优先 `refreshMode = "local"` 或 `RequestLocalRefresh()`；影响显隐、行数或自定义块高度时改走 `RequestPageRebuild()` / `BuildPage()`。
...
| **领域对外 API** | `Toolbox.EJ` 负责锁定、坐骑掉落、入口查找、系统 waypoint 导航和锁定摘要；入口查找按当前 `journalInstanceID` 直接读取 DB 生成的 `Toolbox.Data.NavigationInstanceEntrances` 规范化目标。 |
...
| **列表导航与交互** | 在列表条目右下角提供图钉按钮，点击后查找该条目副本入口、打开入口地图并设置系统用户 waypoint / super tracking；未勾选 `listPinAlwaysVisible` 时图钉仅在鼠标悬停可导航条目时显示，列表单击继续沿用 Blizzard 原生进入详情页行为。`厄运之槌 - 戈多克议会`、`斯坦索姆 - 仆从入口` 等运行时只返回聚合入口或共享入口的条目，由 `navigation_instance_entrances` 契约直接导出精确运行时目标补足。 |
...
| 2026-05-01 | `encounter_journal` 副本入口导航运行时正式收口到 `navigation_instance_entrances` 契约与 `Toolbox.Data.NavigationInstanceEntrances` 单表目标；导出层吸收 `journalinstanceentrance.MapID = 0` 的共享入口来源，`236 / 1292` 直接展开为独立可导航记录，运行时移除静态表 / Blizzard 入口 API 兜底链 |
...
| 2026-04-30 | `navigation` 数字 node ID 收尾：`NavigationRouteEdges` / `NavigationAbilityTemplates` 统一切到不透明数字节点引用，`WalkClusterNodeID` 成为本地步行锚点主字段，`Navigation.lua` 保留 `HearthBindNodeID` 兼容输入并完成逻辑测试迁移 |
| 2026-05-02 | `navigation` 首批 walk component 出口落地：新增 `navigation_walk_components` 契约、`NavigationWalkComponents.lua` 与 runtime 优先消费；本地步行接入和显示代理优先读取 formal component，缺失时继续 fallback 到 `WalkClusterNodeID / WalkClusterKey` |
| 2026-05-02 | `navigation` walk component 方案收口：移除源侧 override 方向，后续 `navigation_walk_components` 只能由正式来源表全自动推导 |
| 2026-04-29 | `settings-host-redesign` 收口：设置宿主改为根类目 + 6 个叶子页，默认打开规则统一为“上次停留叶子页优先，否则回退 `通用`”；Quest Inspector 并回“任务”叶子页工具区 |
| 2026-05-01 | `settings-host-redesign` 第二阶段落地：`SettingsHost` 统一 toggle / choice / menu / multi-select / action / note / custom block helper，模块公共区收口到宿主，`TooltipAnchor` 改为 3 项菜单选择，菜单弹层抬到 `DIALOG` strata 以兼容后续行覆盖与战斗内独立宿主交互 |

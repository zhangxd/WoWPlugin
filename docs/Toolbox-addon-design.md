...
| `Toolbox.DungeonRaidDirectory` | `Core/DungeonRaidDirectory.lua` | 冒险手册驱动的地下城 / 团队副本共享目录：基础记录、支持难度、坐骑掉落摘要、运行时锁定覆盖层、**登录后预热的异步构建状态**、EJ 列表浏览期间的**暂停 / 恢复闸门**，以及设置页重建入口。 |
...
| `Toolbox.MinimapButton` | `Modules/MinimapButton.lua` | `RegisterFlyoutEntry(def)` 供其他模块向小地图按钮悬停菜单追加项；`def` 须含 `id`、`label`/`labelKey`、`onClick`。禁止直接操作 `flyoutRegistry` 或 `flyoutSlotIds`。 |
...
| 小地图打开设置按钮 | `minimap_button` | `modules.minimap_button`（`enabled`/`debug`/`showMinimapButton`/`minimapPos`/`buttonShape`/`flyoutExpand`/`flyoutSlotIds`/`flyoutLauncherGap`/`flyoutPad`/`flyoutGap`） | 独立子页面：启用、调试、清理并重建、是否显示小地图按钮、恢复默认位置；款式（圆/方）、展开方式（纵向/横向）、悬停项顺序与功能池拖放、`flyoutSlotIds`；预览区为居中微缩按钮 + 展开示意（缝宽/内边距/项间距拖动，非游戏内角度）；游戏内沿小地图边缘拖动定角；悬停向左展开（扩展见 `RegisterFlyoutEntry`） |
...
- **每次改动 `ToolboxDB` 结构**（新增键、重命名、移除）必须在 `Core/DB.lua` 的 `DB.Init()` 中写对应迁移代码。
...
6. `Core/Lockouts.lua`、`Core/EncounterJournal.lua`、`Core/MountJournal.lua`、`Core/Item.lua`、`Core/Map.lua` — 领域对外 API
7. `Core/DungeonRaidDirectory.lua` — 冒险手册共享目录（依赖 EJ / Lockouts / MountJournal）
8. `Core/ModuleRegistry.lua`
9. `UI/SettingsHost.lua`
10. `Modules/Mover.lua`、`Modules/MicroMenuPanels.lua`（兼容委托，无 `RegisterModule`）、`Modules/TooltipAnchor.lua`、`Modules/ChatNotify.lua`、`Modules/DungeonRaidDirectory.lua`、`Modules/EJMountFilter.lua`（顺序可按依赖微调）
11. `Core/Bootstrap.lua` — `ADDON_LOADED` 中初始化并启用模块

...
| 2026-04-30 | `navigation` 数字 node ID 收尾：`NavigationRouteEdges` / `NavigationAbilityTemplates` 统一切到不透明数字节点引用，`Navigation.lua` 保留 `HearthBindNodeID` 兼容输入并完成逻辑测试迁移 |
| 2026-05-02 | `navigation` 首批 walk component 出口落地：新增 `navigation_walk_components` 契约、`NavigationWalkComponents.lua` 与 runtime 优先消费；本地步行接入与显示代理改为读取 formal component 与显式 `localEdges`，不再保留 `WalkClusterNodeID / WalkClusterKey` fallback |

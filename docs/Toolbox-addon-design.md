...
├── **Chat（领域门面）** — 默认聊天框输出、TOC 元数据读取；见 `Core/Chat.lua`
├── **Tooltip（领域门面）** — `GameTooltip_SetDefaultAnchor` hook 与光标锚点；见 `Core/Tooltip.lua`
...
└── SavedInstances — 副本锁定、手册掉落扫描、独立窗口（依赖 Mover）
...
| `hooksecurefunc` 等 | 模块内部自行使用；**Tooltip 默认锚点**已集中在 `Core/Tooltip.lua` | 与领域门面重复的 hook 应进门面；其它仍可在模块内使用 |

**领域门面（Domain Facade，与 [AGENTS.md](../AGENTS.md) 一致）**

| 门面 | 文件 | 职责 |
|------|------|------|
| `Toolbox.Chat` | `Core/Chat.lua` | 面向玩家的默认聊天框输出（`PrintAddonMessage`）、插件 TOC 元数据（`GetAddOnMetadata`）。**模块内禁止**直接调用 `DEFAULT_CHAT_FRAME:AddMessage`；新增聊天类能力须先扩展本门面。 |
...
| `Toolbox.Lockouts` | `Core/LockoutsFacade.lua` | 锁定列表：`GetNumSavedInstances` / `GetSavedInstanceInfo` 等封装（至暗之夜仍以此为主流 API，便于日后替换）。 |
| `Toolbox.EJ` | `Core/EncounterJournalFacade.lua` | **优先 `C_EncounterJournal`**，兜底时再考虑全局 `EJ_*`；业务模块禁止直接调用 `EJ_*`。 |
| `Toolbox.MountJournal` | `Core/MountJournalFacade.lua` | `C_MountJournal`（坐骑物品、是否已学会）。 |
| `Toolbox.ItemFacade` | `Core/ItemFacade.lua` | 物品名/链接、`GameTooltip:SetItemByID` 等展示辅助。 |
| `Toolbox.Map` | `Core/MapFacade.lua` | `C_Map.OpenWorldMap` 优先。 |
...
| 类型 | 特征 | 示例 |
|------|------|------|
| **自有 UI** | 只操作插件创建的 Frame | Mover、SavedInstances |
...
| 副本进度 / 掉落浏览 | `saved_instances` | `modules.saved_instances` | 总开关、筛选、手册入口按钮；窗口位置键 `saved_instances`（`modules.mover.frames`） |
...
- **游戏菜单**：在 `GameMenuFrame` 上增加按钮，点击调用 **`Settings.OpenToCategory(categoryID)`**，与 **ESC → 选项 → 插件** 中打开的界面一致。
...
| **门面** | `Core/Tooltip.lua`：`InstallDefaultAnchorHook()`（`hooksecurefunc("GameTooltip_SetDefaultAnchor", ...)`）、`RefreshDriver()`（跟随用 `OnUpdate`）。**勿在 OnUpdate 里 `SetOwner`**，否则会清空提示文字。 |
| **模块** | `tooltip_anchor`：设置 UI 与存档键；调用门面的 `InstallDefaultAnchorHook` / `RefreshDriver`。 |
| **注意** | `UIParent:GetEffectiveScale()`；与暴雪「界面·鼠标」类选项可能叠加。 |

### 5.5 聊天门面（Chat）与加载提示（chat_notify）

| 项 | 说明 |
|----|------|
| **门面** | `Toolbox.Chat`（`Core/Chat.lua`）：`PrintAddonMessage(body)`、`GetAddOnMetadata(name, field)`。 |
...
4. `Core/Chat.lua` — 聊天领域门面  
5. `Core/Tooltip.lua` — 提示框领域门面（须在 `Modules/TooltipAnchor.lua` 之前）  
...
| 2026-04-02 | 领域门面 `Toolbox.Chat`（Core/Chat.lua）；chat_notify 仅调门面；TOC 与 §5.6 补充 |
| 2026-04-02 | 领域门面 `Toolbox.Tooltip`（Core/Tooltip.lua）；tooltip_anchor 变薄；MicroMenuPanels 统一 `getMicroMenuDb()` |

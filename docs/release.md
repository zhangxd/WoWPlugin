...
**新增能力**
- 任务页签升级为左侧资料片树导航，并在资料片下提供 `地图任务线 / 任务类型` 两条浏览路径
- 地图路径主区改为任务线单行列表，点击后原地展开任务列表，再次点击折叠
- 类型视图改为运行时 `questType` 驱动，不再依赖静态导出类型字段
- 任务详情、任务线详情、地图详情统一走 `Toolbox.Questlines` 组装的领域模型

**数据层变更**
- `InstanceQuestlines.lua` 升级为更贴近 DB 的 schema v4 文档结构
- 静态数据层只保留稳定 DB 字段
- 运行时字段与静态字段正式分层

**发布前确认**
- [ ] 游戏内验证资料片树切换、地图/类型双入口、任务线单展开和详情回跳
- [ ] 验证任务页签在无可解析类型时的空态表现
- [ ] 验证 `InstanceQuestlines` 新 schema 的导出、头注释和静态校验

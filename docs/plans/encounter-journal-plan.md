...
## 15. 2026-04-15 执行补充（任务能力拆分为 quest 独立模块）

- 执行状态：可执行（用户已明确回复“开动”）。
- 已确认决策：
  1. 新模块：`moduleId = quest`。
  2. `encounter_journal` 完全移除任务相关能力与设置。
  3. 小地图飞出菜单新增“任务”入口，打开 `quest` 模块主界面。
  4. 任务界面底部分页签仅保留两个：`active_log`（第一个）和 `map_questline`（第二个）。
  5. `map_questline` 保留“资料片 -> 混合列表”视图；`quest_type` 完全移除。
  6. `active_log` 使用系统任务界面样式复刻，不嵌入 Blizzard 原生任务窗口。
  7. 当前任务页签底部显示“历史完成”区域，默认展开。
  8. 导航为独立一行的通用组件：视觉与视图融为一体，但不属于视图内容，由当前页签视图注册路径节点数据。
  9. `active_log` 不注册路径节点；`map_questline` 注册可点击路径节点并支持定位跳转。
  10. `questInspector`（任务详情查询）迁移到 `quest` 模块。
  11. `ToolboxDB.modules.encounter_journal.quest*` 旧键完全移除，不做迁移保留。
- 本轮实施清单：
  - [ ] 新增 `quest` 模块注册、生命周期与设置页骨架。
  - [ ] 将任务视图实现迁入 `quest`，并改造为独立界面（顶部导航行 / 中部主视图 / 底部分页签）。
  - [ ] 调整任务视图模式，仅保留 `active_log` 与 `map_questline`。
  - [ ] 重排 `active_log` 页面结构：当前任务主区 + 底部历史完成（默认展开）。
  - [ ] 迁移 Quest Inspector 到 `quest` 模块。
  - [ ] 更新 `minimap_button` 飞出项，增加“任务”入口并联动打开 `quest` 界面。
  - [ ] 在 `encounter_journal` 中移除任务入口、任务设置、任务事件处理与任务视图刷新路径。
  - [ ] 更新 `Config.lua` 默认键与迁移/清理逻辑（新增 `modules.quest`，清理 `encounter_journal.quest*`）。
  - [ ] 更新 `Toolbox.toc` 加载顺序，确保新模块与依赖正确装载。
  - [ ] 更新并执行相关逻辑测试，覆盖拆分后的关键路径。
- 本轮验收标准：
  1. 冒险指南中不再出现任务页签与任务设置。
  2. 小地图“任务”入口可打开独立任务界面。
  3. 任务界面仅有两个页签且顺序正确。
  4. 导航行为独立组件，`active_log` 无路径节点，`map_questline` 有可点击路径节点。
  5. 当前任务页签中历史完成区默认展开。
  6. `ToolboxDB.modules.encounter_journal.quest*` 旧键被清理且不再被读写。

...
- 设计：
  `docs/designs/encounter-journal-design.md`
- 其他约束：
  当前代码实现是唯一事实来源；任务能力已拆到 `quest` 模块；导航入口已由用户在 2026-04-27 回复“开动”确认，并在后续反馈中修正为副本列表条目右下角图钉。
...
- 修改：
  ...
  - `tests/logic/spec/encounter_journal_tooltip_spec.lua` 或新增同类 `encounter_journal` 逻辑测试
- 验证：
...
- 游戏内验证点：
  副本列表“仅坐骑”、副本列表图钉导航、CD 叠加、详情页增强、小地图与 `EJMicroButton` 锁定摘要。

...
def normalize_achievement_faction_id(faction_id_text: str) -> str:
    if faction_id_text in ("0", "1", "-1"):
        return faction_id_text
    return "-1"


def build_achievement_faction_tags(faction_id_seen: set[str]) -> list[str]:
    if "-1" in faction_id_seen:
        return ["shared"]
    faction_tags: list[str] = []
    if "1" in faction_id_seen:
        faction_tags.append("alliance")
    if "0" in faction_id_seen:
        faction_tags.append("horde")
    if not faction_tags:
        return ["shared"]
    return faction_tags
...
def build_instance_questlines_model(
    ...
    for row in csv_rows:
        ...
        achievement_faction_texts = (row.get("AchievementFactionIDs") or "").split("=")
        ...
        for index, achievement_id_text in enumerate(achievement_id_texts):
            ...
            achievement_faction_text = achievement_faction_texts[index] if index < len(achievement_faction_texts) else ""
            normalized_faction_id = normalize_achievement_faction_id(achievement_faction_text)
            achievement_state["faction_id_seen"].add(normalized_faction_id)
    ...
    for achievement_id, achievement_state in sorted(achievement_aggregate.items()):
        ...
        faction_tags = build_achievement_faction_tags(achievement_state["faction_id_seen"])
    ...
    for achievement_id, achievement_entry in model.achievements.items():
        ...
        lines.extend(
            ...
            + [
                ...
                f"      FactionTags = {format_string_array(achievement_entry.faction_tags)},",

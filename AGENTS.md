...
关 3 · 是否触发"新功能门禁"？
  触发条件（满足任一）：
    ...
    • Toolbox.toc 新增行
  注：在已有模块的 ToolboxDB 键下新增字段，不触发关 3，但须在 DB.lua defaults 中声明并写迁移
...
- **账号级 vs 角色级存档**：`ToolboxDB`（`SavedVariables`）为账号级，所有角色共享，适合窗口位置、UI 偏好、插件开关。如需角色级（`SavedVariablesPerCharacter`），须在 TOC 中声明独立变量名（如 `ToolboxDBChar`）并在 `Core/DB.lua` 中单独初始化。**当前版本不使用角色级存档**；若未来需要，须先更新总设计文档 §3 并经关 3 评审。

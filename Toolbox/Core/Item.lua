--[[
  物品信息门面：展示用同步信息仍以数字 itemID + GetItemInfo 为主（12.0 仍可用）；
  若未来暴雪收紧全局 API，可改为 Item:CreateFromItemID + ContinueOnItemLoad 异步加载。
]]

Toolbox.ItemFacade = Toolbox.ItemFacade or {}

---@param itemID number
---@return string|nil itemName
---@return string|nil itemLink
function Toolbox.ItemFacade.GetItemNameAndLink(itemID)
  if not itemID then
    return nil, nil
  end
  if GetItemInfo then
...
function Toolbox.ItemFacade.SetTooltipItemByID(tooltip, itemID)
  ...
  elseif GameTooltip_ShowCompareItem then
    ...
    local _, link = Toolbox.ItemFacade.GetItemNameAndLink(itemID)
    if link then

...
function Toolbox.Tooltip.RefreshDriver()
  ...
  if db.enabled == false then
    ...
    local fmt = L.TOOLTIP_DEBUG_DRIVER_ON_FMT or "模式: %s"
    debugPrint(string.format(fmt, tostring(mode)))
  end
end
...
function Toolbox.Tooltip.InstallDefaultAnchorHook()
  ...
  hooksecurefunc("GameTooltip_SetDefaultAnchor", function(tooltip, parent)
    ...
    -- 使用 ANCHOR_CURSOR_RIGHT 让提示框显示在光标右下角，支持偏移量
    tooltip:ClearAllPoints()
    tooltip:SetOwner(parent, "ANCHOR_CURSOR_RIGHT", db.offsetX or 0, db.offsetY or 0)
